...
def create_sequences_tech(df, tech_features, target_col, time_steps):
    """
    Build overlapping sequences for technical and macro branches efficiently,
    without reallocating large arrays in each loop.
    Returns:
    ...
    return np.array(X_tech), np.array(y)
...
def build_multi_input_5_layer_lstm_tech(time_steps, n_tech):
    # Technical branch
    tech_in = Input(shape=(time_steps, n_tech), name='tech_input')
    tech_lstm_1 = LSTM(128, return_sequences=True, name='tech_lstm_1')(tech_in)  # Layer 1
    tech_lstm_2 = LSTM(128, return_sequences=False, name='tech_lstm_2')(tech_lstm_1)  # Layer 2
    ...
    # Final output layer (3 classes)
    out = Dense(3, activation='softmax', name='output')(tech_dropout)  # Layer 5

    model = Model(inputs=[tech_in], outputs=out)
    model.compile(
        optimizer='adam',
        loss='sparse_categorical_crossentropy',
        metrics=['accuracy']
...
def main():
    ...
    merged_path = './forex_with_technical_indicators.xlsx'
    ...
    # 3) Label target
    df = label_data(df)
    df.to_excel('./labeld_data_all_tech.xlsx', index=False)
    ...
    time_steps = 30
    X_tech, y = create_sequences_tech(df, tech_features, 'label', time_steps)
    ...
    # Build & train
    model = build_multi_input_5_layer_lstm_tech(time_steps,
                                           len(tech_features)
                                           )
    model.summary()
    ...
    history = model.fit(
        {'tech_input': X_tr_tech_s}, y_train,
        validation_data=({'tech_input': X_te_tech_s}, y_test),
        epochs=50,
        batch_size=32,
        callbacks=[es, rl]
    )
